...
namespace Laboratory.Scripts
{
    ...
    public class PUN_SyncInterpolationBuffer : MonoBehaviourPun, IPunObservable
    {
        ...
        {
            // ประกาศตัวแปรเหล่านี้ไว้ที่นี่ เพื่อให้เข้าถึงได้ทั้งเมธอด
            ...
            else
            {
                ...
                timestamp = info.SentServerTimestamp / 1000.0; // Convert ms to sec.
            }

            // ************** ส่วนที่ต้องแก้ไข (Error Section) **************
            // ส่วนนี้จะทำงานเฉพาะเมื่อ stream.IsReading เป็นจริง และเราได้รับข้อมูล 
            // ดังนั้นเราควรทำการตรวจสอบเฉพาะในกรณีที่เรากำลังอ่านข้อมูล (Remote Client) เท่านั้น
            if (!stream.IsWriting) 
            {
