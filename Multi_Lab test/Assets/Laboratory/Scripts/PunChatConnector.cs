...
public class PunChatConnector : MonoBehaviourPun, IOnEventCallback
{
    ...
    private void HandleUISendRequest(string message, int targetDropdownIndex)
    {
        if (!IsNetworkActive())
        {
            ...
            ChatUIManager.Instance.ReceiveMessage("Me(Offline)", message, "#FFFF00"); // Yellow for offline
            return;
        }
    ...
    public void OnEvent(EventData photonEvent)
    {
        if (photonEvent.Code == CHAT_EVENT_CODE)
        {
            ...
            {
                senderName = string.IsNullOrEmpty(senderPlayer.NickName) ? $"Player {senderID}" : senderPlayer.NickName;
            }
